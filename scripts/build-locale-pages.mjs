#!/usr/bin/env node
/**
 * Generate localized landing pages for book-quote-scanner.app
...
const METADATA_PATH = path.join(ROOT, '../clarify/locales/appStoreMetadata.json');
const PHRASES_PATH = path.join(ROOT, 'locales/landing-phrases.json');
const TEMPLATE_PATH = path.join(ROOT, 'index.html');
const APP_ID = '6744657207';
const APP_SLUG = 'book-quote-scanner';
const BASE_URL = 'https://scanquotes.app';
...
function buildLocalePage(code) {
  ...
  features.push(
    { title: o.feature5Title, desc: o.feature5Desc },
    { title: o.feature6Title, desc: o.feature6Desc },
  );
  ...
  html = html.replace(
    /https:\/\/book-quote-scanner\.app\/assets\//g,
    `${BASE_URL}/assets/`,
  );
  html = html.replace(
    /https:\/\/apps\.apple\.com\/us\/app\/book-quote-scanner\/id6744657207/g,
    storeUrl,
  );
  ...
  html = html.replace(/"url": "https:\/\/book-quote-scanner\.app\/"/g, `"url": "${pageUrl}"`);
  html = html.replace(
    /"name": "Book Quote Scanner"/,
    `"name": "${escapeJson(appName)}"`,
  );
  html = html.replace(
    /"description": "Book quote scanner app for iPhone\. Snap a photo of any page, select quotes, and save them to your personal library with OCR, definitions, and AI vocabulary help\."/,
  ...
  html = html.replace(
    ...
    `<h1 class="app-title">${o.heroH1Before}${titleSep(code, o.heroH1Before, o.heroH1Highlight)}<span class="highlight-quote">${o.heroH1Highlight}</span>${titleSep(code, o.heroH1Highlight, o.heroH1After)}${o.heroH1After}</h1>`,
  );
  ...
  html = html.replace(
    /<span class="title-line">Your personal quote library,<\/span>\s*<span class="title-line title-accent">built from every page you read<\/span>/,
    `<span class="title-line">${o.sectionFeaturesLine1}</span>\n                    <span class="title-line title-accent">${o.sectionFeaturesLine2}</span>`,
  );
  html = html.replace(
    /<p class="section-subtitle">Everything you need to scan, save, and read book quotes again[\s\S]*?<\/p>/,
  ...
  html = html.replace(
    /<p class="section-subtitle">Experience the intuitive interface designed for capturing and saving book quotes<\/p>/,
    `<p class="section-subtitle">${o.sectionScreenshotsSubtitle}</p>`,
  );

  html = html.replace(
    /<h2 class="section-title">What Is a <span class="title-accent">Book Quote Scanner App<\/span>\?<\/h2>/,
  ...
  html = html.replace(
    /<h2 class="section-title">Book Quote Scanner <span class="title-accent">FAQ<\/span><\/h2>/,
  ...
  html = html.replace(
    /<p class="section-subtitle">Common questions about our book quote scanner app for iPhone<\/p>/,
  ...
  html = html.replace(/aria-label="Download Book Quote Scanner on the App Store"/g, `aria-label="${o.downloadAppStore}"`);

  html = html.replace(/<span class="logo-text">Book Quote Scanner<\/span>/, `<span class="logo-text">${appName}</span>`);
